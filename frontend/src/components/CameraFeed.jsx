...
function AppInner() {
  ...
  // Handle view change and keep history
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-r from-purple-400 via-pink-300 to-yellow-200">
      <header className="bg-white shadow-md py-4 relative">
        ...
        {/* Stylish Back Button on top-right */}
        {view !== "landing" && (
          <button
            onClick={goBack}
            className="absolute top-4 right-4 flex items-center px-4 py-2 bg-gradient-to-r from-gray-700 to-gray-900 text-white rounded-full shadow-lg hover:from-gray-800 hover:to-gray-950 transition"
          >
            <span className="mr-2 text-lg font-bold">←</span> Back
          </button>
        )}
      </header>
      ...
      <main className="flex-grow flex items-center justify-center">
        {view === "landing" && (
          <div className="text-center bg-white bg-opacity-80 p-10 rounded-xl shadow-lg">
            <h2 className="text-2xl font-semibold mb-6 text-gray-800">
              Welcome — learn ASL with real-time feedback
            </h2>
            <button
              onClick={() => changeView("learn")}
